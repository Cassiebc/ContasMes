...
export default function Secao({ titulo, itens, onEditar, onRemover, offset, readOnly = false }) {
  ...
  return (
    <section className="mb-7">
      <h2 className="text-[10px] uppercase tracking-[0.3em] text-stone-500 dark:text-stone-400 mb-2">{titulo}</h2>
        ...
        {itens.map((it) => {
          ...
          const detalhes = (
            <>
              ...
              {it.tipo === "parcelado" && (
                <div className="text-xs text-stone-500 dark:text-stone-400 tabular-nums mt-0.5">
          ...
          return (
            <div key={it.id} className="border-b border-stone-300 dark:border-stone-700 py-3 flex items-center gap-3">
              {readOnly ? (
                <div className="flex-1">{detalhes}</div>
              ) : (
                <button onClick={() => onEditar({ ...it })}
                  className="flex-1 text-left focus:outline-none focus:ring-2 focus:ring-stone-800 dark:focus:ring-stone-300">
                  {detalhes}
                </button>
              )}
              ...
              {!readOnly && (
                <button onClick={() => onRemover(it.id)} aria-label={`Remover ${it.nome}`}
                  className="text-stone-400 dark:text-stone-500 px-1 focus:outline-none focus:ring-2 focus:ring-stone-800 dark:focus:ring-stone-300">×</button>
              )}
            </div>
          );
        })}
